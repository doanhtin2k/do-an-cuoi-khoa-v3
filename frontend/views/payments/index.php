                ...
                ?>
                  <table class="table table-bordered">
                      ...
                      <?php foreach ($_SESSION['cart'] AS $product_id => $cart):
                        $product_link ="index.php?controller=product&action=detail&product_id=$product_id";
                        ?>
                          <tr>
                              <td>
                                <?php if (!empty($cart['avatar'])): ?>
                                    <img class="product-avatar img-responsive"
                                         src="../backend/public/uploads/<?php echo $cart['avatar']; ?>" width="60"/>
                                <?php endif; ?>
                              ...
                              <span class="product-price-payment">
                                 <?php echo number_format($cart['price']*(100-$cart['promotions'])/100, 0, '.', '.') ?> vnđ
                              </span>
                              </td>
                              <td>
                              <span class="product-price-payment">
                                  <?php
                                  $price_total = $cart['price'] * $cart['quantity']*(100-$cart['promotions'])/100;
                                  $total += $price_total;
                                  ?>
